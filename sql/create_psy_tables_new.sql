...
CREATE TABLE psy_diplomas (
        gln                 BIGINT UNSIGNED
    ,   professionDe	    VARCHAR(150)
    ,   professionFr	    VARCHAR(150)
    ,   professionIt	    VARCHAR(150)
    ,   professionEn	    VARCHAR(150)
...
CREATE TABLE psy_permissions (
    ...
    ,   PRIMARY KEY (gln, effective_dt, expiry_dt)
    ,   FOREIGN KEY (gln, effective_dt, expiry_dt) 
            REFERENCES psy_cetTitles (gln, effective_dt, expiry_dt) 
            ON DELETE CASCADE
            ON UPDATE CASCADE
);


CREATE TABLE psy_permissionAddress (
        gln                 BIGINT UNSIGNED
    ...
    ,   PRIMARY KEY (gln, effective_dt, expiry_dt)
    ,   FOREIGN KEY (gln, effective_dt, expiry_dt) 
            REFERENCES psy_permissions (gln, effective_dt, expiry_dt) 
            ON DELETE CASCADE
            ON UPDATE CASCADE
);
...
create or replace view psy_effective_merged_nodate_v as (
    ...
    from psy_gln g
        ...
        left join psy_permissions pe            using (gln, effective_dt, expiry_dt)
        left join psy_permissionAddress pa    using (gln, effective_dt, expiry_dt)
